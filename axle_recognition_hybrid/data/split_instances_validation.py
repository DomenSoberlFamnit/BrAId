...
def run(dir_braid, num_training_samples):
    ...
    # Load the predefined validation ids.
    print("Loading the validation ids.")

    predefined_ids = None
    if os.path.exists(f'{dir_braid}validation_set.json'):
        with open(f'{dir_braid}validation_set.json') as file:
            predefined_ids = json.load(file)
        print(f'Found {len(predefined_ids)} predefined testing IDs.')
    else:
        print("No predefined validation ids.")
        quit()
    ...
    for i in range(len(data_y)):
        ...
        if str(id) in predefined_ids:
            groups = predefined_ids[str(id)]
            if groups != 'invalid' and groups != '':
                distribution_test[cls].append(i)
        else:
            distribution[cls].append(i)

    # Print out the statistics.
    print("Distribution of the predefined validation IDs:")
    ...
    print("Setting up the split.")

    training_indices = np.array([], dtype=np.uint32)
    for samples in distribution:
        if samples.size >= num_training_samples:
